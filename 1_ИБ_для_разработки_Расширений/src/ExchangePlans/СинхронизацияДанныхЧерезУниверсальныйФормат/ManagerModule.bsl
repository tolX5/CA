...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	ИначеЕсли ИдентификаторНастройки = "ОбменКасса" Тогда
		...
		ТекстОписания = ТекстОписанияОтборовНаУзлеКассы(НастройкаОтборовНаУзле);
...
Функция ОписаниеЗначенийПоУмолчанию(ЗначенияПоУмолчаниюНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если ЗначенияПоУмолчаниюНаУзле.Свойство("СтатьяПрочихДоходовРасходов_ОприходованиеТоваров") Тогда
	...
	Если ЗначенияПоУмолчаниюНаУзле.Свойство("СкладПоУмолчанию") Тогда
...
Функция ОписаниеОграниченийПередачиДанныхБазыКорреспондента(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если ИдентификаторНастройки = "ОбменКасса"
		ИЛИ НастройкаОтборовНаУзле.ПравилаОтправкиДокументов = "АвтоматическаяСинхронизация" Тогда
...
Функция НастройкаОтборовНаУзле(ИдентификаторНастройки) Экспорт
	...
	Иначе
		СтруктураНастроек.Вставить("ВерсияФорматаОбмена", "1.2");
	КонецЕсли;
	...
	Если ИдентификаторНастройки = "ОбменКасса" Тогда
		...
		СтруктураНастроек.Вставить("ИспользоватьОтборПоСкладам", Истина);
		
		РозничныеСкладыПоУмолчанию = РозничныеСкладыПоУмолчанию();
		
		СтруктураТабличнойЧастиСклады = Новый Структура("Склад", РозничныеСкладыПоУмолчанию.Склады);
		СтруктураНастроек.Вставить("Склады", СтруктураТабличнойЧастиСклады);
		
		СтруктураНастроек.Вставить("ТипЦенДляИзмененияЦен", РозничныеСкладыПоУмолчанию.ТипЦенДляИзмененияЦен);
	...
	Иначе
		...
		СтруктураНастроек.Вставить("ТипЦенДляИзмененияЦен", Справочники.ТипыЦенНоменклатуры.ПустаяСсылка());
	...
	Если ИдентификаторНастройки = "ОбменУТБП" Тогда
		...
		СтруктураНастроек.Вставить("ПравилаОтправкиЦен",          "НеСинхронизировать");
	...
	ИначеЕсли ИдентификаторНастройки = "ОбменКасса" Тогда
		...
		СтруктураНастроек.Вставить("ПравилаОтправкиЦен",          "АвтоматическаяСинхронизация");
		СтруктураНастроек.Вставить("ПравилаОтправкиСправочников", "СинхронизироватьПоНеобходимости");
		СтруктураНастроек.Вставить("ПравилаОтправкиДокументов",   "СинхронизироватьПоНеобходимости"); 
	...
	Если ИдентификаторНастройки = "ОбменУТ11" ИЛИ ИдентификаторНастройки = "ОбменУТБП" Тогда
		...
		СтруктураНастроек.Вставить("ОтправлятьВедомостиНаВыплатуЗарплаты", Ложь);
	ИначеЕсли ИдентификаторНастройки = "ОбменКасса" Тогда
		СтруктураНастроек.Вставить("ОтправлятьДокументыПокупкиПродажи", Ложь);
		СтруктураНастроек.Вставить("ОтправлятьСкладскиеДокументы",      Ложь);
		СтруктураНастроек.Вставить("ОтправлятьАвансовыеОтчеты",         Ложь);
		СтруктураНастроек.Вставить("ОтправлятьСправочники",             Ложь);
		СтруктураНастроек.Вставить("ОтправлятьНоменклатуру",            Истина);
		СтруктураНастроек.Вставить("ОтправлятьБанковскиеДокументы",     Ложь);
		СтруктураНастроек.Вставить("ОтправлятьКассовыеДокументы",       Ложь);
		СтруктураНастроек.Вставить("ОтправлятьВедомостиНаВыплатуЗарплаты", Ложь);
	Иначе
...
Функция НастройкаОтборовНаУзлеБазыКорреспондента(ИдентификаторНастройки)
	...
	Если ИдентификаторНастройки = "ОбменКасса" Тогда
		СтруктураНастроек.Вставить("ДатаНачалаВыгрузкиДокументов", НачалоМесяца(ТекущаяДатаСеанса()));
		СтруктураНастроек.Вставить("ВыгружатьЧекиКакОтчетОРозничныхПродажах", Истина);
		СтруктураНастроек.Вставить("ПравилаОтправкиСправочников",  "СинхронизироватьПоНеобходимости");
	...
	Иначе
		...
		СтруктураНастроек.Вставить("РежимВыгрузкиПриНеобходимости",        Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости);
		СтруктураНастроек.Вставить("РежимВыгрузкиСправочников",            Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПриНеобходимости);
		СтруктураНастроек.Вставить("УправленческаяОрганизация",            Неопределено);
		СтруктураНастроек.Вставить("ВыгружатьАналитикуПоСкладам",          Истина);
		СтруктураНастроек.Вставить("ПравилоОтбораСправочников",            "БезОтбора");
		СтруктураНастроек.Вставить("СкладПоУмолчанию",                     Справочники.Склады.ПустаяСсылка());
...
Функция ВыполнитьПереходНаСинхронизациюДанныхЧерезУниверсальныйФормат(НастройкаСинхронизацииДанных) Экспорт
	...
	НоваяНастройкаОбъект.ВерсияФорматаОбмена = "1.2";
...
Процедура ЗаполнитьЗначенияРеквизитовФильтрации() Экспорт
	...
	|ГДЕ
	|	НЕ Таб.ЭтотУзел
	|	И Таб.ВариантНастройки <> ""ОбменУТБП""
	|	И (Таб.ОтправлятьДокументыПокупкиПродажи = ЛОЖЬ
	|			ИЛИ Таб.ОтправлятьСкладскиеДокументы = ЛОЖЬ
	|			ИЛИ Таб.ОтправлятьАвансовыеОтчеты = ЛОЖЬ)";
	
	ВыборкаУзлов = Запрос.Выполнить().Выбрать();
	Пока ВыборкаУзлов.Следующий() Цикл
		
		УзелПланаОбменаОбъект = ВыборкаУзлов.Ссылка.ПолучитьОбъект();
		УзелПланаОбменаОбъект.ОтправлятьДокументыПокупкиПродажи = Истина;
		УзелПланаОбменаОбъект.ОтправлятьСкладскиеДокументы = Истина;
		УзелПланаОбменаОбъект.ОтправлятьАвансовыеОтчеты    = Истина;
		УзелПланаОбменаОбъект.ОтправлятьБанковскиеДокументы = Истина;
		УзелПланаОбменаОбъект.ОтправлятьКассовыеДокументы   = Истина;
		УзелПланаОбменаОбъект.ОтправлятьВедомостиНаВыплатуЗарплаты = Истина;
...
Процедура ЗаполнитьЗначенияРеквизитовФильтрацииКасса() Экспорт
	...
	"ВЫБРАТЬ
	|	Таб.Ссылка,
	|	Таб.ВариантНастройки,
	|	Таб.ПравилаОтправкиДокументов,
	|	Таб.РежимВыгрузкиДокументов
	|ИЗ
	|	ПланОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат КАК Таб
	|ГДЕ
	|	(Таб.ОтправлятьСправочники = ЛОЖЬ
	|			ИЛИ Таб.ОтправлятьНоменклатуру = ЛОЖЬ
	|			ИЛИ Таб.ОтправлятьБанковскиеДокументы = ЛОЖЬ
	|			ИЛИ Таб.ОтправлятьКассовыеДокументы = ЛОЖЬ
	|			ИЛИ Таб.ОтправлятьВедомостиНаВыплатуЗарплаты = ЛОЖЬ
	|			ИЛИ Таб.ПравилаОтправкиЦен = """")";
	
	УзелЭтойБазы = ПланыОбмена.СинхронизацияДанныхЧерезУниверсальныйФормат.ЭтотУзел();
	
	ВыборкаУзлов = Запрос.Выполнить().Выбрать();
	Пока ВыборкаУзлов.Следующий() Цикл
		
		Если ВыборкаУзлов.Ссылка = УзелЭтойБазы Тогда
			Продолжить;
		КонецЕсли;
		
		Если ВыборкаУзлов.ВариантНастройки = "ОбменКасса" Тогда
			Продолжить;
		КонецЕсли;
		
		УзелПланаОбменаОбъект = ВыборкаУзлов.Ссылка.ПолучитьОбъект();
		УзелПланаОбменаОбъект.ОтправлятьСправочники  = Истина;
		УзелПланаОбменаОбъект.ОтправлятьНоменклатуру = Истина;
		УзелПланаОбменаОбъект.ОтправлятьБанковскиеДокументы = Истина;
		УзелПланаОбменаОбъект.ПравилаОтправкиЦен = ВыборкаУзлов.ПравилаОтправкиДокументов;
		УзелПланаОбменаОбъект.РежимВыгрузкиЦен   = ВыборкаУзлов.РежимВыгрузкиДокументов;
		УзелПланаОбменаОбъект.ОтправлятьКассовыеДокументы   = Истина;
		УзелПланаОбменаОбъект.ОтправлятьВедомостиНаВыплатуЗарплаты = Истина;
...
Функция ТекстОписанияОтборовНаУзлеКассы(Настройка) Экспорт
	
	ОписаниеОтборов = "";
	Если Настройка.Склады.Склад.Количество() = 0 Тогда
		ОписаниеОтборов = ОписаниеОтборов + Символы.Таб + НСтр("ru = 'Не заданы розничные магазины, с которыми будет выполняться синхронизация'");
	Иначе
		ОписаниеСкладов = "";
		НомерСклада = 0;
		
		Для Каждого Склад Из Настройка.Склады.Склад Цикл
			НомерСклада = НомерСклада + 1;
			Если НомерСклада > 2
				И Настройка.Склады.Склад.Количество() > 3 Тогда
				ОписаниеСкладов = ОписаниеСкладов + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'и еще %1 шт.'"), Строка(Настройка.Склады.Склад.Количество() - НомерСклада + 1));
				Прервать;
			Иначе
		...
		ОписаниеОтборов = ОписаниеОтборов + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция РозничныеСкладыПоУмолчанию()
